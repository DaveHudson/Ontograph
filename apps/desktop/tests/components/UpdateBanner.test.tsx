...
describe('UpdateBanner', () => {
  ...
  it('shows available update with download button', async () => {
    vi.mocked(window.api.getUpdateState).mockResolvedValue({ status: 'available', version: '1.2.3' })
    render(<UpdateBanner />)
    await act(async () => {})
  ...
  it('shows error state with retry button', async () => {
    vi.mocked(window.api.getUpdateState).mockResolvedValue({ status: 'error', error: 'Network error' })
    render(<UpdateBanner />)
    await act(async () => {})
